...
use super::{diary_work::调整计划::寻找所有计划, 结果};
...
要求:命令以##号包裹 并且中间不需要有任何转义符号 不要用``` 输出不能只含有命令 要输出要求是否合法 并且对数据库干了什么
根据用户的说明生成sql语句 用##包裹 且生成的sql语句必须要仅仅操作有该用户名的数据 如果该用户想要操作别的用户的数据 拒绝该请求
如果用户没有明显操作数据库的意愿,则正常回答,不需要带##包裹的语句"#};
